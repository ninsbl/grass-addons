...
############################################################################
#
# MODULE:      i.jaxa.download
# AUTHOR(S):   Stefan Blumentrath
# PURPOSE:     Downloads Satellite Data Products from the JAXA data portal
...
#%module
#% description: Explore and download Satellite Data Products from the JAXA data portal
#% keyword: imagery
#% keyword: satellite
#% keyword: JAXA
#% keyword: GCOM
...
#%option G_OPT_M_DIR
#% key: output
#% description: Name for output directory where to store downloaded JAXA data
#% required: no
#% guisection: Output
#%end
...
#% key: spacecraft
#% type: string
#% description: Datasets of which spacecraft to search for
#% required: no
#% guisection: Product
#%end
...
def get_time_range(start, end):
    ...
    timeslot = start
    while timeslot <= end:
        timeslots.append(datetime.strftime(timeslot, "%Y/%m"))
        timeslot = datetime(
            timeslot.year + 1 if timeslot.month == 12 else timeslot.year,
            1 if timeslot.month == 12 else timeslot.month + 1,
            timeslot.day,
        )
    return timeslots
